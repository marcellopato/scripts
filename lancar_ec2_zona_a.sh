...
security_group_id=$(aws ec2 describe-security-groups --group-names "executiva-local" --query "SecurityGroups[0].GroupId" --output text)
...
--tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=executiva-local}]' \
